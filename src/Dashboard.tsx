...
export default function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => setError('Kunde inte hämta data. Kontrollera din anslutning.'))
  }, [])
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-5xl mx-auto px-8 py-8 flex flex-col gap-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiCard label="Totalt utestående" value={overview ? fmt(overview.totalOutstanding) : '—'} />
          <KpiCard label="Förfallet" value={overview ? fmt(overview.totalOverdue) : '—'} highlight />
          <KpiCard label="Snitt betalningstid" value={overview ? `${overview.avgPaymentDays} dagar` : '—'} />
        ...
        <div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            {overview ? (
              overview.topLatePayors.slice(0, 5).map((p, i) => (
              ...
              <div className="px-5 py-8 text-center text-gray-600 text-sm">Laddar...</div>
            )}
          </div>
        </div>

  ...
  return (
    <div className="font-sans">
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-10 flex flex-col gap-10">
        ...
        <div>
          <h1 className="text-4xl tracking-tight text-ink-900">Administration</h1>
          <p className="text-sm text-ink-500 mt-1">Systemöversikt och hantering</p>
        </div>
                ...
                <table className="w-full text-base min-w-[700px]">
                  <thead>
                    <tr className="border-b border-ink-100 text-sm text-ink-400 uppercase tracking-wide">
                      ...
                      <th className="text-left px-5 py-4 font-semibold">Bransch</th>
                      <th className="text-right px-5 py-4 font-semibold">Användare</th>
                      <th className="text-right px-5 py-4 font-semibold">Transaktioner</th>
                      <th className="text-left px-5 py-4 font-semibold">Skapad</th>
                      <th className="text-left px-5 py-4 font-semibold">Senast aktiv</th>
                      ...
                      return (
                        <Fragment key={org.id}>
                          <tr
                            ...
                            className={`cursor-pointer transition-colors ${i !== 0 ? 'border-t border-ink-50' : ''} ${expanded ? 'bg-brand-50/40' : 'hover:bg-ink-50/60'}`}
                          >
                            <td className="px-5 py-4 font-semibold text-ink-900">
                              <span className="flex items-center gap-2">
                                <span className={`text-ink-400 text-xs transition-transform duration-200 ${expanded ? 'rotate-90' : ''}`}>▶</span>
                                {cap(org.name)}
                              </span>
                            </td>
                            ...
                            <tr className="border-t border-brand-100">
                              <td colSpan={7} className="px-5 py-4 bg-brand-50/30">
                                ...
                                {orgUsers.length > 0 ? (
                                  ...
                                  <p className="text-xs text-ink-400">Inga matchande användare i listan.</p>
                                )}
                              </td>
                            </tr>
                          )}
                ...
                <table className="w-full text-base min-w-[560px]">
                  <thead>
                    <tr className="border-b border-ink-100 text-sm text-ink-400 uppercase tracking-wide">
                      ...
                      <th className="text-left px-5 py-4 font-semibold">Organisation</th>
                      <th className="text-left px-5 py-4 font-semibold">Roll</th>
                      <th className="text-left px-5 py-4 font-semibold">Skapad</th>
                      <th className="px-5 py-4 font-semibold text-right">Admin</th>
                    </tr>
                  </thead>
                  <tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-900/20 backdrop-blur-md" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white/60 backdrop-blur-3xl border border-ink-200/60 rounded-2xl shadow-xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-2xl text-ink-900">Ta bort organisation?</h2>
...
function SearchInput({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder: string }) {
  return (
    <div className="relative">
      <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-ink-400 pointer-events-none" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">

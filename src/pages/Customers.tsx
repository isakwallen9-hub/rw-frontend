...
export default function Customers() {
  ...
  return (
    <div className="font-sans">
          ...
          <div className="bg-caution-50/70 backdrop-blur border border-caution-200/70 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {customerRisk.impact?.amount != null && (
                <p className="text-xs text-caution-700 mt-0.5">
            ...
            <button
              ...
              className="shrink-0 flex items-center gap-2 text-xs font-semibold text-caution-700 border border-caution-300 rounded-2xl px-4 py-3 hover:bg-caution-100 transition-colors whitespace-nowrap min-h-[44px]"
            >
        ...
        {!loading && !error && top3.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {top3.map((c, i) => (
                <div key={c.name} className="glass rounded-2xl p-5 flex flex-col gap-3 shadow-sm relative overflow-hidden">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <span className={`text-xs font-bold uppercase tracking-widest ${i === 0 ? 'text-caution-600' : i === 1 ? 'text-ink-500' : 'text-caution-700'}`}>
                        #{i + 1}
                      </span>
                      <p className="text-sm font-bold text-ink-900 mt-0.5 leading-tight">{c.name}</p>
                    </div>
                  ...
                  {c.paymentBehavior === 'inactive' && (
                    <div className="mt-1 flex flex-col gap-2">
                      <button
                        ...
                        className="flex items-center gap-2 text-xs font-semibold text-brand-600 border border-brand-200 rounded-2xl px-3 py-2 hover:bg-brand-50 transition-colors text-left disabled:opacity-60"
                      >
                      ...
                      <button
                        ...
                        className="text-xs text-ink-400 hover:text-ink-700 transition-colors text-left"
                      >
                        Skicka manuellt
                      </button>
                      <button
                        onClick={() => openAiWith(`Hur vinner jag tillbaka ${c.name}?`)}
                        className="flex items-center gap-2 text-xs font-semibold text-ink-400 border border-ink-200 rounded-2xl px-3 py-2 hover:bg-ink-50 transition-colors"
                      >
                        <Sparkles className="w-3 h-3 shrink-0" aria-hidden="true" />
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-4 py-3 border border-ink-200 rounded-2xl text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/30 transition-colors bg-white"
              />
            </div>
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="border border-ink-200 rounded-2xl px-3 py-3 text-sm text-ink-700 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/30 transition-colors bg-white"
              >
                {SORT_OPTIONS.map((o) => (
        ...
        {loading ? (
          ...
          <div className="glass rounded-2xl overflow-hidden">
            ...
            {sorted.map((c, i) => (
              <div
                key={c.name}
                className={`flex flex-col sm:grid sm:grid-cols-[1fr_130px_100px_130px_130px_110px_150px] sm:items-center gap-2 sm:gap-4 px-5 py-4 transition-colors hover:bg-ink-50/50 ${i < sorted.length - 1 ? 'border-b border-ink-50' : ''}`}
              >
                <span className="font-semibold text-ink-900 text-sm">{c.name}</span>
                <span className="sm:text-right text-sm font-bold text-primary tabular-nums whitespace-nowrap">
                  <span className="sm:hidden text-xs text-ink-400 font-normal mr-1">Inflöde:</span>
                  {fmt(c.totalRevenue)}
                </span>
                <span className="sm:text-right text-sm text-ink-700 tabular-nums whitespace-nowrap">
                  ...
                  {c.paymentBehavior === 'inactive' && (
                    <>
                      <button
                        onClick={() => void openAiReminder(c)}
                        disabled={aiReminderLoading === c.name}
                        className="flex items-center gap-2 text-xs font-semibold text-brand-600 border border-brand-200 rounded-2xl px-3 py-2 hover:bg-brand-50 transition-colors whitespace-nowrap disabled:opacity-60"
                      >
                        {aiReminderLoading === c.name ? (
                          <span className="w-3 h-3 border-2 border-brand-500 border-t-transparent rounded-full animate-spin shrink-0" />
                        ) : (
      ...
      {/* Reminder modal */}
      {reminderCustomer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-900/40 backdrop-blur-sm" onClick={closeReminder}>
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={closeReminder} className="text-ink-400 hover:text-ink-700 text-2xl leading-none">&times;</button>
            </div>
            <p className="text-sm text-ink-400 mb-3">{reminderCustomer.name}</p>

            {reminderIsAiPrepared && (
              <div className="flex items-center gap-2 bg-brand-50 border border-brand-100 rounded-2xl px-4 py-3 mb-4">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                <label className="block text-xs font-medium text-ink-500 mb-1">
                  Personligt meddelande <span className="text-ink-400">(valfritt)</span>
                </label>
                <textarea
                  ...
                  rows={3}
                  className="w-full border border-ink-200 rounded-2xl px-4 py-3 text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/30 transition-colors resize-none"
                />
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button
                ...
                className="flex-1 py-3 text-sm font-semibold text-white bg-negative-600 rounded-2xl hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
              >
      ...
      {/* Success toast */}
      {reminderToast && (
        <div className="fixed bottom-6 left-6 z-50 flex items-center gap-2 bg-positive-700 text-white text-sm font-semibold px-4 py-3 rounded-2xl shadow-lg">
...
function BehaviorBadge({ behavior }: { behavior: PaymentBehavior }) {
  ...
  return (
    <span className={`inline-block text-xs font-semibold px-3 py-1 rounded-full ${cfg.className}`}>
      {cfg.label}
    </span>
  )
}
...
function EmptyState({ hasSearch }: { hasSearch: boolean }) {
  if (hasSearch) {
    return (
      <div className="glass rounded-2xl px-6 py-12 flex flex-col items-center text-center gap-2">
        <p className="text-ink-500 text-sm">Inga kunder matchade din sökning.</p>
      </div>
    )
  }
  return (
    <div className="glass rounded-2xl px-6 py-12 flex flex-col items-center text-center gap-3">
      <svg className="w-10 h-10 text-ink-400" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
      </svg>
      <p className="text-ink-900 font-semibold text-sm">Inga kundnamn hittades i din data.</p>

        ...
        {!loading && !error && top3.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {top3.map((c, i) => (
                <div key={c.name} className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-xl p-5 flex flex-col gap-3 shadow-sm relative overflow-hidden">
...
function EmptyState({ hasSearch }: { hasSearch: boolean }) {
  if (hasSearch) {
    return (
      <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-xl px-6 py-12 flex flex-col items-center text-center gap-2">
        <p className="text-gray-500 text-sm">Inga kunder matchade din sökning.</p>
      </div>
    )
  }
  return (
    <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-xl px-6 py-12 flex flex-col items-center text-center gap-3">

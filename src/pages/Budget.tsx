    ...
    <div className="glass rounded-2xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-2xl bg-ink-50 flex items-center justify-center text-ink-500">{icon}</div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-ink-50 rounded-2xl px-4 py-3">
          <p className="text-xs font-bold uppercase tracking-widest text-ink-500 mb-1">Budget</p>
          <p className="text-sm font-bold text-ink-900 tabular-nums">{line.budget > 0 ? fmt(line.budget) : '—'}</p>
        </div>
        <div className="bg-ink-50 rounded-2xl px-4 py-3">
          <p className="text-xs font-bold uppercase tracking-widest text-ink-500 mb-1">Faktiskt</p>
          <p className="text-sm font-bold text-ink-900 tabular-nums">{fmt(line.actual)}</p>
        </div>
      </div>
      {line.budget > 0 && (
        <div className="mb-3">
          <div className="flex items-center justify-between text-xs text-ink-400 mb-1.5">
            <span>Utfall</span>
            <span className="font-semibold tabular-nums">{fillPct.toFixed(0)}%</span>
          </div>
      ...
      {line.budget > 0 && (
        <div className={`flex items-center gap-2 text-sm font-semibold ${positive ? 'text-positive-600' : 'text-negative-600'}`}>
          {positive ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
          <span className="tabular-nums">{diff >= 0 ? '+' : ''}{fmt(diff)} ({fmtPct(line.actual, line.budget)})</span>
    ...
    <div className="font-sans">
      <div className="max-w-2xl mx-auto px-4 sm:px-8 py-8 flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1 bg-white border border-ink-200 rounded-2xl shadow-sm px-2 py-2 self-start sm:self-auto">
            <button onClick={prevMonth} className="w-8 h-8 rounded-2xl flex items-center justify-center text-ink-500 hover:bg-ink-100 hover:text-ink-900 transition-colors" aria-label="Föregående månad">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="text-sm font-semibold text-ink-900 px-3 min-w-[130px] text-center">{periodLabel(period)}</span>
            <button onClick={nextMonth} disabled={isCurrentMonth} className="w-8 h-8 rounded-2xl flex items-center justify-center text-ink-500 hover:bg-ink-100 hover:text-ink-900 transition-colors disabled:opacity-30 disabled:cursor-not-allowed" aria-label="Nästa månad">
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="flex items-center gap-3 mb-5">
            <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center shrink-0">
              <Sparkles className="w-4 h-4 text-white" aria-hidden="true" />
            </div>
            <div>
              <p className="text-sm font-semibold text-ink-900">Låt AI:n sätta din budget</p>
          ...
          {!aiLoading && !aiProposal && (
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-4 bg-brand-600 text-white text-sm font-semibold rounded-2xl hover:bg-brand-700 transition-colors min-h-[44px]"
            >
          ...
          {/* Loading */}
          {aiLoading && (
            <div className="flex items-center gap-3 py-4 text-ink-700">
          ...
          {aiProposal && !aiLoading && (
            <div className="flex flex-col gap-4">
              ...
              {/* AI badge */}
              <div className="flex items-center gap-2 bg-brand-50 border border-brand-100 rounded-2xl px-4 py-3">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <div className="bg-positive-50/80 border border-positive-200/80 rounded-2xl p-4">
                  <p className="text-xs font-bold uppercase tracking-wider text-positive-600 mb-1.5 flex items-center gap-1">
                  ...
                  {aiProposal.revenueReasoning && (
                    <p className="text-xs text-ink-700 leading-relaxed">{aiProposal.revenueReasoning}</p>
                  )}
                </div>
                <div className="bg-caution-50/80 border border-caution-200/80 rounded-2xl p-4">
                  <p className="text-xs font-bold uppercase tracking-wider text-caution-700 mb-1.5 flex items-center gap-1">
                  ...
                  {aiProposal.costReasoning && (
                    <p className="text-xs text-ink-700 leading-relaxed">{aiProposal.costReasoning}</p>
                  )}
                </div>
              </div>
              ...
              {aiProposal.focusTip && (
                <div className="bg-brand-50 border border-brand-200 rounded-2xl p-4 flex gap-3">
                  <div className="w-7 h-7 rounded-2xl bg-brand-100 flex items-center justify-center shrink-0 mt-0.5">
                    <Lightbulb className="w-3.5 h-3.5 text-brand-600" aria-hidden="true" />
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-brand-600 mb-1">Fokus för att nå målet</p>
              ...
              {aiSaved && (
                <div className="bg-positive-50 border border-positive-200 rounded-2xl px-4 py-3 flex items-center gap-2">
              ...
              {!aiSaved && (
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    onClick={() => void runAiProposal()}
                    disabled={aiLoading}
                    className="px-4 py-3 border border-ink-200 text-ink-700 text-sm font-medium rounded-2xl hover:border-ink-300 hover:text-ink-900 transition-colors min-h-[44px]"
                  >
                    Prova igen
                  </button>
                </div>
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm font-medium text-ink-500 hover:text-ink-900 transition-colors self-start"
        >
        ...
        {manualOpen && (
          <div className="glass rounded-2xl shadow-sm p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <div>
                <h2 className="text-base text-ink-900">Sätt budget manuellt</h2>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-5">
              <div>
                ...
                />
                <p className="text-xs text-ink-400 mt-1">Förväntat totalt inflöde</p>
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full border border-ink-200 rounded-2xl px-4 py-3 text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/30 transition-colors"
                />
                <p className="text-xs text-ink-400 mt-1">Max tillåtet utflöde</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                className="flex items-center gap-2 px-5 py-3 bg-primary text-white text-sm font-semibold rounded-2xl shadow-md hover:opacity-90 transition-opacity disabled:opacity-50 min-h-[44px]"
              >
                {saving ? (
                  <>
                    <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin shrink-0" />
              ...
              {savedMsg && (
                <div className="flex items-center gap-2 text-positive-700 text-sm font-medium">
        ...
        <div>
          <h2 className="text-2xl text-ink-900 mb-3">Budget vs utfall: {periodLabel(period)}</h2>
              ...
              {[0, 1].map(i => (
                <div key={i} className="glass rounded-2xl p-5 shadow-sm">
                  <div className="space-y-3 animate-pulse">
                    <div className="h-4 bg-ink-100 rounded-2xl w-1/2" />
                    <div className="h-10 bg-ink-100 rounded-2xl" />
                    <div className="h-2.5 bg-ink-100 rounded-full" />
                    <div className="h-4 bg-ink-100 rounded-2xl w-2/3" />
                  </div>
                </div>
              ))}
            </div>
          ) : fetchError ? (
            <div className="bg-negative-50 border border-negative-100 text-negative-600 rounded-2xl px-5 py-4 text-sm">{fetchError}</div>
          ) : data ? (
            <>
              <div className="grid sm:grid-cols-2 gap-4">
              ...
              {(() => {
                ...
                return (
                  <button
                    ...
                    className="mt-2 w-full flex items-center gap-2 bg-white/40 backdrop-blur border border-ink-200/60 rounded-2xl px-4 py-3 text-sm font-medium text-ink-700 hover:bg-white/60 hover:border-brand-300 hover:text-brand-700 transition-[transform,box-shadow,background-color,border-color,color,opacity] min-h-[44px]"
                  >
        ...
        {data && (data.revenue.budget > 0 || data.costs.budget > 0) && (
          <div className="glass rounded-2xl shadow-sm p-5">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-ink-500 mb-1">Netto budget</p>
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-ink-500 mb-1">Netto faktiskt</p>
                <p className={`text-sm font-bold tabular-nums ${(data.revenue.actual - data.costs.actual) >= 0 ? 'text-positive-600' : 'text-negative-600'}`}>
                  {fmt(data.revenue.actual - data.costs.actual)}
                </p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-ink-500 mb-1">Avvikelse</p>
                {(() => {
                  ...
                  return (
                    <p className={`text-sm font-bold tabular-nums ${diff >= 0 ? 'text-positive-600' : 'text-negative-600'}`}>

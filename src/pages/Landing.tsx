    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" aria-hidden="true">
      <defs>
        <linearGradient id="rwHeroArea" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#3A5CD8" stopOpacity="0.26" />
          <stop offset="100%" stopColor="#3A5CD8" stopOpacity="0" />
        </linearGradient>
      </defs>
      ...
      <path
        ...
        className="rw-draw"
      />
      <circle cx={last.x} cy={last.y} r="4.5" fill="#3A5CD8" className="rw-area-in" />
      <circle cx={last.x} cy={last.y} r="8" fill="#3A5CD8" fillOpacity="0.18" className="rw-area-in" />
    </svg>
  )
}

/** Tiny static sparkline for feature cells. */
function Sparkline({ series, className = '', stroke = '#3A5CD8' }: {
    ...
    <div className="relative min-h-screen font-sans overflow-x-hidden">
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 pt-16 pb-20 sm:pt-24 sm:pb-28">
        <div className="lg:grid lg:grid-cols-12 lg:gap-14 lg:items-center">
          ...
          <div className="lg:col-span-6">
            <FadeIn>
              <div className="inline-flex items-center gap-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-ink-500">
            ...
            <FadeIn delay={90}>
              <h1 className="mt-6 text-[3.25rem] leading-[1.02] sm:text-[5rem] sm:leading-[0.98] text-ink-900 text-balance">
            ...
            <FadeIn delay={170}>
              <p className="mt-6 text-lg text-ink-500 leading-relaxed max-w-md">
                ...
                För dig som vill <span className="text-ink-700 font-medium">förstå</span> siffrorna, inte bara bokföra dem.
              </p>
            </FadeIn>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2.5 text-sm text-ink-500">
                {['Gratis under beta', 'Setup under 5 min', 'Data lagrad i Sverige'].map((t) => (
                  <span key={t} className="inline-flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-brand-600/80 shrink-0" aria-hidden="true" />
                    {t}
                  </span>
                ))}
              </div>
                ...
                <div className="relative glass-kpi rounded-3xl p-5 sm:p-7 shadow-xl">
                  <div className="flex items-center justify-between mb-5">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-semibold text-ink-500">Netto denna månad</p>
                    </div>
                    <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-positive-700 bg-positive-50 border border-positive-100 rounded-full px-2.5 py-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-positive-500 motion-safe:animate-pulse" aria-hidden="true" />
                      LIVE
                    </span>
                  </div>

                  <div className="flex items-end gap-2">
                    <span className="text-4xl sm:text-5xl font-extrabold text-positive-600 tabular-nums tracking-tight leading-none">
                      +{cashflow.toLocaleString('sv-SE')}
                    </span>
                    <span className="text-lg font-bold text-positive-600/70 mb-0.5">kr</span>
                    <span className="ml-auto inline-flex items-center gap-1 text-sm font-bold text-positive-600 mb-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-ink-200/45">
              {[
                ...
                <div key={s.label} className="bg-white/55 backdrop-blur px-5 py-6 sm:px-6 sm:py-7">
                  <p className="text-3xl sm:text-4xl font-extrabold text-ink-900 tabular-nums tracking-tight leading-none">
                    {s.big}<span className="text-lg font-bold text-ink-400 ml-1">{s.unit}</span>
                  </p>
                  <p className="mt-2 text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-ink-400">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 py-20 sm:py-28">
        <FadeIn className="max-w-xl mb-12">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-600 mb-3">Allt på ett ställe</p>
          <h2 className="text-[2.25rem] sm:text-5xl sm:leading-[1.02] text-ink-900 text-balance">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
            ...
            <div className="group relative h-full glass rounded-3xl p-7 sm:p-8 overflow-hidden hover:-translate-y-1 hover:shadow-xl transition-[transform,box-shadow,background-color,border-color,color,opacity] duration-500">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-start gap-6">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="mt-2 text-ink-500 leading-relaxed max-w-sm">
          ...
          <FadeIn delay={80}>
            <div className="group h-full glass rounded-3xl p-7 flex flex-col hover:-translate-y-1 hover:shadow-xl transition-[transform,box-shadow,background-color,border-color,color,opacity] duration-500">
              ...
              <p className="mt-2 text-sm text-ink-500 leading-relaxed">In- och utflöden per kategori, period och kund.</p>
              <Sparkline series={[30, 45, 38, 55, 48, 66, 60]} className="mt-auto pt-5 w-full h-9 text-brand-600" />
            </div>
          </FadeIn>
          ...
          <FadeIn delay={160} className="lg:col-span-3">
            <div className="group glass rounded-3xl p-7 flex flex-col sm:flex-row sm:items-center gap-5 hover:-translate-y-1 hover:shadow-xl transition-[transform,box-shadow,background-color,border-color,color,opacity] duration-500">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-ink-500 leading-relaxed">Exportera till CSV och dela snygga rapporter med revisorn, på ett klick.</p>
              </div>
              <Link
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 pb-24">
        <FadeIn>
          <div className="relative glass rounded-[2rem] overflow-hidden px-6 py-14 sm:px-16 sm:py-20">
            ...
            <Sparkline
              series={HERO_SERIES}
              className="absolute inset-x-0 bottom-0 w-full h-40 text-brand-500/10"
            />
            <div className="relative max-w-xl">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-600 mb-4">Kom igång idag</p>
              <h2 className="text-[2.5rem] sm:text-6xl text-ink-900 leading-[1.02] text-balance">
                Redo att se klart?
              </h2>
                ...
                {loggedIn ? (
                  ...
                  <>
                    ...
                    <span className="inline-flex items-center gap-2 text-sm text-ink-500">
                      <Clock className="w-4 h-4 text-brand-600/80" aria-hidden="true" />
      ...
      <footer className="border-t border-white/50 bg-white/40 backdrop-blur">
        <div className="max-w-6xl mx-auto px-4 sm:px-8 py-9 flex flex-col sm:flex-row items-center justify-between gap-5">
          ...
          <p className="text-xs text-ink-400">© 2026 RW Systems. Alla rättigheter förbehållna.</p>
        </div>
      </footer>
    </div>
  )

...
function NavItem({ to, label, icon: Icon, onNav }: NavItem & { onNav?: () => void }) {
  return (
    <NavLink
      ...
      onClick={onNav}
      className={({ isActive }) =>
        `relative flex items-center gap-3 px-3 py-2 rounded-2xl text-sm font-medium transition-[transform,box-shadow,background-color,border-color,color] duration-200 ease-apple min-h-[44px] ${
          isActive
  ...
  return (
    <div className="flex flex-col h-full select-none">
      ...
      <div className="px-4 pt-5 pb-3 shrink-0">
        <NavLink to="/dashboard" className="flex items-center gap-3 group" onClick={onNav}>
          <div className="w-7 h-7 rounded-2xl bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center shrink-0 shadow-sm">
            <span className="text-white text-xs font-black tracking-tight">RW</span>
          </div>
      ...
      <nav className="flex-1 overflow-y-auto px-2 pb-4" aria-label="Huvudmeny">
        {NAV_GROUPS.map((g, gi) => (
          <div key={g.label}>
            <p className={`text-xs font-bold uppercase tracking-[0.12em] text-ink-400 px-3 mb-1.5 ${gi === 0 ? 'mt-4' : 'mt-1 pt-4 border-t border-ink-200/50'}`}>
              {g.label}
            </p>
            {g.items.map(item => (
      ...
      <div className="px-2 pb-5 shrink-0 border-t border-ink-200/50 pt-3">
        ...
        <button
          ...
          className="flex items-center gap-3 px-3 py-2 rounded-2xl text-sm font-medium text-ink-700 hover:bg-negative-50/60 hover:text-negative-700 transition-colors w-full mt-1 min-h-[44px]"
        >
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-40 p-2 rounded-2xl bg-white/80 backdrop-blur-xl border border-ink-200/60 shadow-sm"
      ...
      {mobileOpen && (
        <>
          <div
            ...
            aria-hidden="true"
          />
          <aside className="lg:hidden fixed inset-y-0 left-0 w-72 bg-white/95 backdrop-blur-2xl border-r border-ink-200/60 z-50 shadow-lg flex flex-col">
            <div className="absolute top-3.5 right-3.5">
              <button
                onClick={() => setMobileOpen(false)}
                className="p-2 rounded-2xl text-ink-500 hover:bg-ink-100 transition-colors"
